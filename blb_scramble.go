...
func Descramble(buf []byte) error {
	...
	need := 0x20 + n // 32 + n
	if len(Blb3ShiftRow) < need {
		return fmt.Errorf("Blb3ShiftRow too short: have=%d need>=%d (bufLen=%d)", len(Blb3ShiftRow), need, n)
	}
